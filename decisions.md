...
## 2026-09-09 · Architecture diagram added to /how-it-works

Supplied a hand-built SVG (`arch.svg`, generated by an accompanying Python
script, not pasted from the prototype convention this project otherwise
follows) tracing the whole pipeline in one picture: user message -> extract
-> cumulative slot state -> derive constraints -> fork into SQL-vs-retrieval
-> generate, with a "machinery trace" side-rail calling out that every
stage is logged and shown live in the demo. Saved verbatim to
`app/public/architecture-diagram.svg` and embedded on `/how-it-works` via a
plain `<img>` in a new section ("The whole path, end to end") placed
between the budget-slider section and the interactive six-stage tracker --
establish the big picture first, then let the reader drill into it stage
by stage.

The SVG's colors happen to be hand-matched to this project's own light-
theme tokens (`#FCFDFD`, `#131A1B`, `#155E63`, ...) and it paints its own
opaque background as the first shape in the file, so it doesn't need any
dark-mode handling -- it renders as a self-contained light card regardless
of the surrounding page's theme, the same way a screenshot would, and was
verified to look intentional (not broken) in both. Used a plain `<img>`
rather than `next/image`: this is a one-off static local SVG, and the app
has no other `next/image` usage to match, so the added config complexity
(SVGs need `unoptimized` or `dangerouslyAllowSVG`) wasn't worth it for a
single diagram -- accepted the resulting lint warning (not an error) over
that.

### Verification

`npx tsc --noEmit`, `eslint` (one expected `no-img-element` warning, zero
errors), and `npm run build` all clean. Live-verified with Playwright
against a production server in both `colorScheme: "light"` and `"dark"`:
confirmed the image element is present and actually loaded
